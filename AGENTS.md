...
| [source-weighting](references/source-weighting.md) | `Source hierarchy` principle — load when reasoning about how to weight curated repo sources vs. training knowledge, or when cross-domain borrowing is in play |
...
- **Source hierarchy — opinion and recommendation tasks:** When forming any view or recommendation, treat curated repo sources (skills/, references/, knowledge/) as the primary layer — consult them before drawing conclusions. Do not treat them as a ceiling: your full training knowledge remains in scope and should be used to fill gaps, extend reasoning, and make cross-domain connections. Cross-domain borrowing is valuable when same-domain material does not adequately cover the case; it is a liability when it substitutes for suitable material that is already available. For mechanical execution tasks (renaming, formatting, running commands, following a clear spec) this hierarchy does not apply. When a recommendation draws on multiple layers, be explicit about which layer each part came from.
